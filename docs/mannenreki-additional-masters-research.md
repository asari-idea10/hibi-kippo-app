...
| right | 一 | `月合` | 月徳合 | confirmed | `pilot-a-right.png` | 字形と原位置を確認 |
...
| right | 七 | `月合` | 月徳合 | confirmed | `pilot-b-right.png` | 字形と原位置を確認 |
...
| left | 三 | なし | なし | confirmed | `pilot-b-whole.png` | 原位置を確認 |
| upper_left | 四 | なし | なし | confirmed | `pilot-b-whole.png` | 原位置を確認 |
...
| upper_right | 八 | `冲`, `破` | 定位対冲、月破 | confirmed | `pilot-c-whole.png` | 字形と原位置を確認 |
| right | 四 | なし | なし | confirmed | `pilot-c-whole.png` | 大字四のみ。小記号なし |
...
| left | 九 | なし | なし | confirmed | `pilot-c-whole.png` | 原位置を確認 |
| upper_left | 一 | なし | なし | confirmed | `pilot-c-whole.png` | 原位置を確認 |
...
## 22. Step 5B-3C-3 p.24 Level 2 initial evidence display

Status: `display_only`, `pilot_only`, `photo_confirmed`, `production_rule_unconnected`

- Initial display scope is limited to five independently reread Tiger-pilot records: A `天合/bottom`, A `冲/lower_left`, B `天合/bottom`, C `天合/bottom`, and C `冲/upper_right`.
- All records retain raw labels, page-relative positions, source `HMA-P24-IMG-20260715`, and the specific p.24 asset IDs used for the plate and side legend.
- `天合→天徳合` and `冲→定位対冲` are shown only as confirmed side-legend correspondences. No monthly direction-deity calculation or effect is derived from those labels.
- `direction8` and `mountain24` are fixed to `null`. The UI says `紙面下` / `紙面左下` / `紙面右上` and never converts these records to compass directions.
- Every selected-date monthly-board card shows `月盤原典の照合状況`. It labels the Level 1 scope as `基本盤：照合済み` and lists 中宮・九星配置・五黄殺・暗剣殺・月破 without changing those calculations.
- Tiger-pilot matches show `追加marker：Pilot確認あり` and the existing marker details. Other months show `追加marker：この月は未照合` and `細字方位神は寅月Pilotから順次確認中`; this is not a claim that the source contains no markers.
- The display module is separate from monthly Level 1 provenance and is not imported by candidate, ranking, warning, API, direction-deity, direction-compass, or 24-mountain modules.
- C寅月徳合, `天月`, source `三合`, the remaining 296 transcribed cells, source orientation 5/9, and 24-mountain positions remain outside this display scope and unresolved where previously recorded.
